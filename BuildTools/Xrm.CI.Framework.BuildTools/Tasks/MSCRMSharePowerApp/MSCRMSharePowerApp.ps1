...
Write-Verbose 'Entering MSCRMSharePowerApp.ps1'
...
{
	$appsToShareJson = (Get-Content "$shareConfigFile" -Raw)
}
else
{
	$appsToShareJson = "{`"AppSharing`":[{`"AppName`":`"$AppName`",`"ShareWith`":[{`"PrincipalType`":`"$principalType`",`"Principal`":`"$principal`",`"RoleName`":`"$roleName`"}]}]}"
}

& "$mscrmToolsPath\xRMCIFramework\9.0.0\SharePowerApps.ps1" .\SharePowerApps.ps1 -TenantId $TenantId -ApplicationId $ApplicationId -ApplicationSecret $ClientSecret -EnvironmentUrl "$EnvironmentUrl" -AppsToShareJson $appsToShareJson -MSALModulePath $msalPSPath -MGUsersModulePath $mgUsersPath -MGGroupsModulePath $mgGroupsPath -PowerAppsAdminModulePath $powerAppsAdminPath -CrmConnectorModulePath $CrmConnectorPath
